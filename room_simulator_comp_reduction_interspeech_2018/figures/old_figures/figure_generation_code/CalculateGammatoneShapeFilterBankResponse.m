% TODO(chanwcom)
% This will be reimplemented as a classdef using the Factor pattern.
%
...
%
function [H] = ComputeFilterResponse(... 
  ...
  if include_pi
    ...
    H = zeros(fft_size / 2 + 1, length(gain));
  end
  ...
  for chan = 1 : length(gain)
    ...
    if include_pi 
      H(1 : fft_size / 2, channel_index)  = H1 .* H2 .* H3 .* H4;
      H(fft_size / 2 + 1, channel_index) = ... 
          H1_highest * H2_highest * H3_highest * H4_highest;
    else
      H(:, channel_index)  = H1 .* H2 .* H3 .* H4;
    end
  end
end
