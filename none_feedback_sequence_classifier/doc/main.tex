...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \For {epoch index $i = 0, . . . , I-1$}
      \For {each batch $\left\{\mathcal{X}[0:M],
                        \mathcal{Y}_{0:L}  \right\}$}, 
...
\begin{algorithm}
  ...
  \Require {$\mathcal{X}[0:M]$}
  \Require {$\mathcal{Y}_{0:L}$}
  \Require {The neural network parameter $\Theta^{\left(i\right)}$ 
    for calculating 
      $p\left(\vec{z}[m] \big|
      \mathcal{Z}[0:m], \mathcal{X}[0:m+1]\right)$}

  \State {\textbf{Initialization:}}
  \State {Let $\mathcal{Z}_0[0:1] := \left\{ \vec{y}_0 \right\}  $}
  \State {Let $\mathcal{L}_{0} := \ln \left[ 
        p \left(\vec{y}_{0} \big| 
            {\mathcal{X}}[0:1]\right)  \right]$}
  \State {$\mathcal{Z}_b[0:1], 1 \le b < N_b$ are undefined.}
  \For {$m = 1, . . . , M-1$}
    \For {$b = 0, . . . , N_b-1$}
      \If {$\mathcal{Z}_{b}[0:m]$ is defined}
        \State {Let $\vec{y}_u := \vec{z}_{b}[m-1] $}
        \State {Let $\mathcal{Z}_{b, 0}[0:m+1] := \mathcal{Z}_{b}[0:m] 
        ...
        \State {
              $\mathcal{L}_{b,0} := \ln \left[ 
                  p \left(\vec{y}_{u} \big| 
                      \mathcal{Z}_b[0:m], 
                      \mathcal{X}[0:m+1], \Theta^{(i)}\right)  \right]
                  + \mathcal{L}_b$}

        \If {$u+1 \le  L - 1$}
          \State {Let $\mathcal{Z}_{b, 1}[0:m+1] 
          := {\mathcal{Z}}_{b}[0:m] \cup \left\{ \vec{y}_{u+1}  \right\}  $} 
    ...
    $\left\{\mathcal{L}_{b, i}, {\mathcal{Z}}_{b, i}[0:m+1] \right\}, \; 0 \le
    b < N_b, i = 0, 1$ 
    ...
    follows: 
    $\left\{\mathcal{L}_{b}, \mathcal{Z}_{b}[0:m+1] \right\}$, $0 \le b < N_b$}
