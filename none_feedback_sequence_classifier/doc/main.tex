...
\begin{algorithm}
  ...
  \Require {$\vec{\mathcal{X}}[0:M]$}
  \Require {$\mathcal{\vec{Y}}_{0:L}$}
  \Require {The neural network parameter $\Theta^{\left(i\right)}$ for calculating 
      $p\left(\vec{z}[m] \big|
      \mathcal{\vec{Z}}[0:m], \mathcal{\vec{X}}[0:m+1]\right)$}

  \State {\textbf{Initialization:}}
  \State {Let $\vec{Z}_0[0:1] := \left\{ \vec{y}_0 \right\}  $}
  \State {$\vec{Z}_b[0:1], 1 \le b < N_b$ are undefined.}
  \State {Let $\mathcal{L}_{0} := \ln \left[ 
        p \left(\vec{y}_{0} \big| 
            \vec{\mathcal{X}}[0:1]\right)  \right]$}
  \For {$m = 1, . . . , M-1$}
    \For {$b = 0, . . . , N_b-1$}
      \If {$\vec{\mathcal{Z}}_{b}[0:m]$ is defined}
        \State {Let $\vec{y}_u := \mathcal{\vec{Z}}_{b}[m-1] $}
        \State {Let $\vec{\mathcal{Z}}_{b, 0}[0:m+1] := \vec{\mathcal{Z}}_{b}[0:m] 
                \cup \left\{ \vec{y}_u  \right\}  $} 
            \Comment {A horizontal move $P \rightarrow Q$ in Fig. \ref{fig:path_alignment}}.
        \State {
              $\mathcal{L}_{b,0} := \ln \left[ 
                  p \left(\vec{y}_{u} \big| 
                      \vec{\mathcal{Z}}_b[0:m], 
                      \vec{\mathcal{X}}[0:m+1], \Theta^{(i)}\right)  \right]
                  + \mathcal{L}_b$}

        \If {$u+1 \le  L - 1$}
          \State {Let $\vec{\mathcal{Z}}_{b, 1}[0:m+1] := \vec{\mathcal{Z}}_{b}[0:m] \cup \left\{ \vec{y}_{u+1}  \right\}  $} 
              \Comment {A diagonal move $P \rightarrow R$ in Fig. \ref{fig:path_alignment}}.
          \State  {
              $\mathcal{L}_{b,1} := \ln \left[ 
                  p \left(\vec{y}_{u+1} \big| 
                      \vec{\mathcal{Z}}_b[0:m], \vec{\mathcal{X}}[0:m+1], \Theta^{(i)}\right)  \right] 
                          + \mathcal{L}_b$}
      \EndIf
      \EndIf
    \EndFor
    \State {Sort a list of items
    $\left\{\mathcal{L}_{b, i}, \vec{\mathcal{Z}}_{b, i}[0:m+1] \right\}, \; 0 \le b < B, i = 0, 1$ 
    with respect to the log likelihood $\left\{\mathcal{L}_{b, i}\right\}$ 
    in the decreasing order. The top $N_b$ of this sorted list is chosen and re-indexed as 
    follows: 
    $\left\{\mathcal{L}_{b}, \vec{\mathcal{Z}}_{b}[0:m+1] \right\}$, $0 \le b < N_b$}
